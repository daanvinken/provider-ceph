...
type CephUserObservation struct {
	// BackendStatuses is a map of the s3 backends on which the CephUser
...
type CephUserSpec struct {
	...
	// S3 backends on which the CephUser is to be created.
	Providers   []string           `json:"providers"`
	ForProvider CephUserParameters `json:"forProvider"`
	// Disabled allows the user to create a CephUser CR without creating
	// CephUsers on any S3 backends. If an existing CephUser CR is updated
	// with Disabled=true, then s3 attempts to remove any
	// existing CephUsers from the existing S3 backends and the CephUser
